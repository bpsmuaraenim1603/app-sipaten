<!-- resources/views/admin/discipline-criteria/create.blade.php -->
<x-main-layout>
    <x-slot name="header">
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('discipline.criteria.store') }}" method="POST">
                        @csrf
                        <div class="space-y-4">
                            <div>
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md ...">{{ old('description') }}</textarea>
                            </div>
                            <div class="flex items-center">
                                <input type="checkbox" name="is_active" id="is_active" value="1" checked class="h-4 w-4 ...">
                                <label for="is_active" class="ml-2 block text-sm text-gray-900">Aktifkan kriteria ini</label>
                            </div>
                        </div>
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('discipline.criteria.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150 mr-4">
                                Batal
                            </a>
